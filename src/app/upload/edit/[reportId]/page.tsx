...
const STEPS = [
  ...
  {
    id: 'product_mix', label: 'Product Mix Toast', icon: '🍽️', system: 'Toast', required: false,
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <header className="border-b border-gray-800 bg-gray-900 px-6 py-4 flex items-center gap-4">
        <button onClick={() => router.push('/dashboard/history')} className="text-gray-400 hover:text-white text-sm">
          ← Historial
        </button>
        <div>
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
          ...
          {STEPS.map((s, i) => (
            <div key={s.id} onClick={() => setCurrentStep(i)}
              className={`h-2 flex-1 rounded-full transition-all cursor-pointer ${
                ...
                i === currentStep ? 'bg-blue-400' : 'bg-gray-800'
              }`}
              title={s.label}
            />
          ))}
        ...
        <div className="flex items-center gap-4 mb-6 text-xs text-gray-500">
          ...
          <div className="flex items-center gap-1.5"><div className="w-3 h-2 rounded-full bg-blue-500" /><span>Nuevo archivo seleccionado</span></div>
