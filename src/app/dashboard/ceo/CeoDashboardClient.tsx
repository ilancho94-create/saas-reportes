...
export type CeoInitialData = {
  allRestData: any[]
  sortedWeeks: string[]
}

export default function CeoDashboardClient({ initialData }: { initialData: CeoInitialData }) {
  const { currentOrganization } = useAuth()
  const [selectedRestaurant, setSelectedRestaurant] = useState<string>('all')
  const [activeTab, setActiveTab] = useState<Tab>('resumen')
  const [selectedWeek, setSelectedWeek] = useState('')

  // SWR + fallbackData: la 1ra render usa los datos del Server Component,
  // así no hay spinner ni waterfall. Background revalidation cuando stale.
  const { data, isLoading } = useSWR(
    ['ceo-data', currentOrganization?.id || 'default'],
    fetchCeoData,
    {
