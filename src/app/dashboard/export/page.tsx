...
export default function ExportPage() {
  const { currentRestaurant, organizations } = useAuth()
  ...
  async function loadWeeks(restaurantId: string) {
    ...
    setAllWeeks(data || [])
    if (data?.length) setSelectedWeek(data[0].week)
  }
  ...
  async function uploadLogo(file: File) {
    ...
    } catch (e: any) { setError('Error subiendo logo: ' + e.message) }
  ...
  async function saveTemplate() {
    ...
    await supabase.from('export_templates').insert({
      name: newTemplateName, restaurant_id: currentRestaurant?.id,
      organization_id: currentRestaurant?.organization_id,
      is_system_template: false, template_type: 'pptx',
      ...
      file_path: customLogoUrl || null, created_by: user?.id,
    })
    setNewTemplateName('')
    loadSavedTemplates()
  ...
  function moveSectionDown(key: string) {
    setSections(prev => {
      const idx = prev.indexOf(key); if (idx >= prev.length - 1) return prev
      const next = [...prev]; [next[idx], next[idx+1]] = [next[idx+1], next[idx]]; return next
    })
  }

  // Semana seleccionada + anterior automáticamente
  const selectedWeeks = (() => {
    if (!selectedWeek || !allWeeks.length) return []
    const idx = allWeeks.findIndex(w => w.week === selectedWeek)
    const weeks = [selectedWeek]
    if (idx < allWeeks.length - 1) weeks.push(allWeeks[idx + 1].week)
    return weeks
  })()

  const prevWeek = selectedWeeks[1] || null

  async function generate(format: 'pptx' | 'xlsx' | 'pdf') {
    if (!selectedRestaurants.length || !selectedWeeks.length) {
      setError('Selecciona al menos un restaurante y una semana'); return
    }
    setGenerating(true); setGeneratingFormat(format); setError('')
    try {
      const config: ExportConfig = {
        restaurantIds: selectedRestaurants, weeks: selectedWeeks,
        sections: sections.filter(s => ALL_SECTIONS.some(a => a.key === s)),
        notes, template: selectedTemplate, format, language: 'es',
      }
      const dataArr = await Promise.all(selectedRestaurants.map(id => fetchExportData(id, selectedWeeks)))
      const validData = dataArr.filter(Boolean) as any[]
      if (!validData.length) { setError('No se encontraron datos'); setGenerating(false); return }
      if (format === 'pptx') {
    ...
    } catch (e: any) { setError('Error generando reporte: ' + e.message); console.error(e) }
    setGenerating(false); setGeneratingFormat('')
  }

  const STEPS: { id: Step; label: string; num: number }[] = [
    { id: 'restaurantes', label: 'Restaurantes', num: 1 },
    { id: 'periodo', label: 'Semana', num: 2 },
  ...
  function canProceed(s: Step): boolean {
    ...
    if (s === 'periodo') return !!selectedWeek
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <div className="border-b border-gray-800 bg-gray-900 px-6 py-4">
        ...
        <p className="text-gray-500 text-xs mt-0.5">Genera presentaciones y reportes con datos reales</p>
      </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-6 space-y-6">
        ...
        {step === 'restaurantes' && (
          <div className="space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <p className="text-gray-500 text-xs mb-4">¿Individual o multi-restaurante?</p>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h2 className="text-white font-semibold mb-4">Seleccionar restaurante</h2>
                ...
                <div key={org.id}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-4">
                    {org.restaurants.map(rest => (
                      <button key={rest.id}
                        onClick={() => { reportMode === 'individual' ? setSelectedRestaurants([rest.id]) : toggleRestaurant(rest.id); loadWeeks(rest.id) }}
        ...
        {step === 'periodo' && (
          <div className="space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h2 className="text-white font-semibold mb-2">Seleccionar semana</h2>
              <p className="text-gray-500 text-xs mb-5">La semana anterior se incluye automáticamente para comparación.</p>
              <select value={selectedWeek} onChange={e => setSelectedWeek(e.target.value)}
                className={inputCls + ' w-full max-w-md'}>
                {allWeeks.map(w => (
                  <option key={w.week} value={w.week}>{w.week}  ({w.week_start} → {w.week_end})</option>
                ))}
              </select>
              {selectedWeek && (
                <div className="mt-4 flex items-center gap-3 bg-gray-800 rounded-xl px-4 py-3 w-fit">
                  <div>
                    <p className="text-xs text-gray-500 mb-0.5">Semana actual</p>
                    <p className="text-blue-400 font-semibold text-sm">{selectedWeeks[0]}</p>
                  </div>
                  {prevWeek && <>
                    <span className="text-gray-600">vs</span>
                    <div>
                      <p className="text-xs text-gray-500 mb-0.5">Semana anterior</p>
                      <p className="text-gray-400 text-sm">{prevWeek}</p>
                    </div>
                  </>}
                  {!prevWeek && <p className="text-gray-600 text-xs">Sin semana anterior disponible</p>}
                </div>
              )}
            </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setSections(ALL_SECTIONS.map(s => s.key))} className="text-xs text-blue-400 hover:text-blue-300">Todas</button>
                  ...
                  return (
                    <div key={key} className="bg-gray-800 border border-gray-700 rounded-xl p-3">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <button onClick={() => toggleSection(key)} className="w-5 h-5 rounded border-2 border-blue-500 bg-blue-500 flex items-center justify-center shrink-0">
              ...
              {ALL_SECTIONS.filter(s => !sections.includes(s.key)).length > 0 && (
                <div className="mt-4 pt-4 border-t border-gray-800">
                  <p className="text-gray-600 text-xs mb-2">No incluidas:</p>
        ...
        {step === 'template' && (
          <div className="space-y-4">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {savedTemplates.map(t => (
                    <button key={t.id} onClick={() => setSelectedTemplate({ id: t.id, name: t.name, colorPrimary: t.color_primary, colorSecondary: t.color_secondary, colorAccent: t.color_accent, logoUrl: t.file_path || undefined })}
                      className={`p-4 rounded-xl border-2 text-left transition ${selectedTemplate.id === t.id ? 'border-blue-500' : 'border-gray-700 hover:border-gray-600'}`}>
                      <div className="flex items-center gap-2 mb-2">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h2 className="text-white font-semibold mb-4">Logo del restaurante</h2>
              ...
              <div className="border-t border-gray-800 pt-4">
                ...
                <div className="flex gap-2">
                  <input type="text" placeholder="Nombre del template" value={newTemplateName}
                    onChange={e => setNewTemplateName(e.target.value)} className={`flex-1 ${inputCls}`} />
            ...
            <div className="flex justify-between">
              ...
              <button onClick={() => setStep('preview')} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg text-sm font-medium transition">
                Ver resumen →
              </button>
            </div>
          </div>
        ...
        {step === 'preview' && (
          <div className="space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-800 rounded-xl p-4">
                  <p className="text-gray-500 text-xs mb-1">Restaurante</p>
                  <p className="text-white font-bold text-sm mt-1">{allRestaurants.filter(r => selectedRestaurants.includes(r.id)).map(r => r.name).join(', ')}</p>
                </div>
                <div className="bg-gray-800 rounded-xl p-4">
                  <p className="text-gray-500 text-xs mb-1">Semana</p>
                  <p className="text-blue-400 font-bold text-sm mt-1">{selectedWeeks[0]}</p>
                  {prevWeek && <p className="text-gray-500 text-xs mt-0.5">vs {prevWeek}</p>}
                </div>
                <div className="bg-gray-800 rounded-xl p-4">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <p className="text-gray-500 text-xs mb-6">El archivo se descargará automáticamente</p>
